...
removeItem(index: number): void {
  ...
  createCouvertureReseau(): void {
    if (this.userForm.valid) {
      const couvertureReseauxList = this.userForm.value.items.map(
        (item: any) => ({
          pourcentage: item.pourcentage,
          typeReseau: {
            id: item.typeReseauId,
            type: "",
            couvertureReseauxList: [],
          },
        })
      );

      this.programmeService
        .createCouvertureReseaux(couvertureReseauxList)
        .subscribe(
          (response) => {
  ...
  createNewInfrastructurestype(): void {
    ...
    this.programmeService.addType(newInfrastructurestype).subscribe(
      ...
      (error) => {
        console.error("Error creating new Type Infrastructures:", error);
        alert(
          "Error creating new Type Infrastructures. Please try again later."
        );
      }
    );
  }
  addIntervention(): void {
    if (this.userForm.valid) {
      const infrastructurestypeList = this.userForm.value.items.map(
        (item: any) => ({
          quantites: item.quantites,
          infrastructurestype: {
            id: item.infrastructurestypeId,
            type: "",
            infrastructurestypeList: [],
          } as Infrastructurestype,
        })
      );
  ...
  onTypeReseauChange(event: any, index: number): void {
    ...
    if (this.couvertureReseaux[index]) {
      ...
      if (this.couvertureReseaux[index].typeReseau) {
        ...
        this.couvertureReseaux[index].typeReseau = {
          id: selectedTypeReseauId,
          type: "",
          couvertureReseauxList: [],
        };
      }
  ...
  addLogementTypique() {
    this.logementsTypiques.push(
      new LogementTypique(LogementType.MAISON, 0, Qualite.BONNE)
    );
  }

  addLogementModerne() {
    this.logementsModernes.push(
      new LogementModerne(LogementType.APPARTEMENT, 0, Qualite.BONNE)
    );
  }
  ...
  saveLogementsTypiques() {
    ...
    forkJoin(saveObservables).subscribe(
      ...
      (error) => {
        console.error(
          "Erreur lors de la sauvegarde des logements typiques",
          error
        );
      }
    );
  ...
  saveLogementsModernes() {
    ...
    forkJoin(saveObservables).subscribe(
      ...
      (error) => {
        console.error(
          "Erreur lors de la sauvegarde des logements modernes",
          error
        );
      }
    );
  ...
  createQuartier(): void {
    ...
    if (this.quartierName.trim() !== "" && this.selectedCommuneId !== 0) {
      const selectedCommune = this.communes.find(
        (c) => c.id === this.selectedCommuneId
      );
  ...
  getQuartiers(): void {
    this.programmeService
      .getAllQuartiers()
      .subscribe((quartiers) => (this.Quartiers = quartiers));
  }
  ...
  validateInput(event: KeyboardEvent): void {
    ...
    if (!regex.test(inputChar)) {
      ...
      this.alphaNumericErrorMessage =
        "Seules les lettres et les espaces sont autorisés.";
  ...
  loadDelegationsByGouvernaurat(gouvernoratId: number): void {
    ...
    this.programmeService.getDelegationsByGouvernaurat(gouvernoratId).subscribe(
      (delegations: Delegation[]) => {
  ...
  loadCommunesByDelegation(delegationId: number): void {
    ...
    this.programmeService.getCommunesByDelegation(delegationId).subscribe(
      (communes: Commune[]) => {
  ...
  createCommune(): void {
    if (this.communeName && this.selectedDelegationId !== 0) {
      const newCommune: Commune = {
        ...
        delegation: {
          id: this.selectedDelegationId,
          name: "",
          gouvernorat: { id: 0, name: "" },
        },
      };
  ...
  deleteCommune(id: number): void {
    this.programmeService.deleteCommune(id).subscribe(
      ...
      (error) => {
        ...
        console.error(
          `Une erreur est survenue lors de la suppression du programme avec l'ID ${id}:`,
          error
        );
      }
    );
  ...
  getAllProgrammes(): void {
    this.programmeService.getAllProgrammes().subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          "Une erreur est survenue lors de la récupération des programmes:",
          error
        );
      }
    );
  ...
  getProgrammeById(id: number): void {
    this.programmeService.getProgrammeById(id).subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          `Une erreur est survenue lors de la récupération du programme avec l'ID ${id}:`,
          error
        );
      }
    );
  ...
  updateProgramme(id: number, updatedProgramme: Programme): void {
    this.programmeService.updateProgramme(id, updatedProgramme).subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          `Une erreur est survenue lors de la mise à jour du programme avec l'ID ${id}:`,
          error
        );
      }
    );
  ...
  getAllGouvernorats(): void {
    this.programmeService.getAllGouvernorats().subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          "Une erreur est survenue lors de la récupération des gouvernorat:",
          error
        );
      }
    );
  ...
  getGouvernoratById(id: number): void {
    this.programmeService.getGouvernoratById(id).subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          `Une erreur est survenue lors de la récupération du gouvernorat avec l'ID ${id}:`,
          error
        );
      }
    );
  ...
  createGouvernorat(newGouvernorat: Gouvernorat): void {
    this.programmeService.createGouvernorat(newGouvernorat).subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          "Une erreur est survenue lors de la création du gouvernorat:",
          error
        );
      }
    );
  ...
  updateDelegation(delegation: Delegation): void {
    this.programmeService.updateDelegation(delegation.id, delegation).subscribe(
      (updatedDelegation) => {
        console.log(
          `Délégation avec l'ID ${updatedDelegation.id} mise à jour avec succès.`
        );
        // Remplacez la délégation mise à jour dans la liste des délégations
        const index = this.delegations.findIndex(
          (d) => d.id === updatedDelegation.id
        );
        if (index !== -1) {
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          `Une erreur est survenue lors de la mise à jour de la délégation avec l'ID ${delegation.id}:`,
          error
        );
      }
    );
  ...
  deleteGouvernoratById(id: number): void {
    this.programmeService.deleteGouvernoratById(id).subscribe(
      () => {
        ...
        this.gouvernorats = this.gouvernorats.filter(
  ...
  updateGouvernorat(id: number, nouveauNom: string): void {
    ...
    this.programmeService.updateGouvernorat(id, updateGouvernorat).subscribe(
      ...
      (error) => {
        // Gérez les erreurs ici
        console.error(
          `Une erreur est survenue lors de la mise à jour du gouvernorat avec l'ID ${id}:`,
          error
        );
      }
    );
  ...
  initializeMap() {
    const osmTileLayer = L.tileLayer(
      "https://mts1.google.com/vt/lyrs=y@186112443&hl=x-local&src=app&x={x}&y={y}&z={z}&s=Galile",
      {
        attribution:
